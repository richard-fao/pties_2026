...
def load_example_data(filepath='Calificaciones_2026.xlsx') -> pd.DataFrame:
...
with tabs[0]:
    st.markdown(
    "⚡ **Filtros globales:** Todos los filtros que selecciones (Evaluación, IEM, Grado y Área) afectan **toda la información mostrada en los gráficos y tablas a continuación**. "
    ...
    if selected_iem == 'Todas':# and selected_municipio == 'Todos':
        df_box = df_filtered.groupby(['NOMBRE IEM', 'NUM_DOCUMENTO', 'GRADO'])['CALIFICACION'].sum().reset_index()
        fig_box = px.box(
            df_box, x='NOMBRE IEM', y='CALIFICACION', color='NOMBRE IEM', points='all',
        ...
        fig_hist = px.histogram(
            df_box, x='NOMBRE IEM', y='CALIFICACION', color='GRADO', barmode='group',
        ...
        df_box = df_filtered.groupby(['NOMBRE IEM', 'NUM_DOCUMENTO', 'GRADO', 'EVALUACION'])['CALIFICACION'].sum().reset_index()
        fig_box = px.box(
